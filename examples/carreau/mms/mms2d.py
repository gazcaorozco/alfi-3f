...
#        base = Mesh(os.path.dirname(os.path.abspath(__file__)) + "/square_mine.msh",
                    distribution_parameters=distribution_parameters)
        return base

    def bcs(self, Z):
        bcs_ = [DirichletBC(Z.sub(0), Constant((0.,0)), 1),
                DirichletBC(Z.sub(0), Constant((0., 0.)), 2),
                DirichletBC(Z.sub(0), Constant((0., 0.)), 3),
                DirichletBC(Z.sub(0), self.exact_velocity(Z), 4)] #mesh: 1-left, 20-right, 3-bottom, 4-top
        return bcs_
...
class CarreauMMS_Lup(NonNewtonianProblem_Lup):
    def __init__(self, baseN, diagonal=None, **params):
        super().__init__(**params)
        if diagonal is None:
            diagonal = "left"
...
class CarreauMMS_LSup(NonNewtonianProblem_LSup):
    def __init__(self, baseN, diagonal=None, **params):
        super().__init__(**params)
        if diagonal is None:
            diagonal = "left"
