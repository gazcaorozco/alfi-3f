...
class NonNewtonianSolver(object):
    ...
    def __init__(self, problem, nref=1, solver_type="almg",
        ...
        if scalar_conv == "none":
            scalar_conv = None
        assert scalar_conv in {None, "natural_Ra", "natural_Ra2", "natural_Gr", "forced", "forced2"}, "Invalid thermal convection regime %s" % scalar_conv
    ...
    def solve(self, param, info_param, predictor="trivial"):
        ...
        if self.nsp is not None:
            ...
            if self.formulation_Sup or self.formulation_Tup or self.formulation_Lup:
                ...
                (_, p) = self.z.split()
            elif self.formulation_TSup or self.formulation_LSup:
                (_, _, _, p) = self.z.split()
